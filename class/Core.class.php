...
class Core {
    ...
    public static function getPreferedDomain() {
        return Config::get('global', 'domain');
    }
    ...
    public static function getRequest() {
        ...
        //$_SERVER['SERVER_NAME'] => config Apache
        //$_SERVER['HTTP_HOST'] => URL du client
        if ($_SERVER['HTTP_HOST'] != self::getPreferedDomain()) {
            header("HTTP/1.0 301 Moved Permanently");
            header("Location: " . self::getProto() . "://" . self::getPreferedDomain() . $_SERVER['REQUEST_URI']);
            die;
        }
