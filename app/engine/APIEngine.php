<?php
    ...
    class APIEngine
    {
        ...
        public static function start($module, $function, $param)
        {
            ...
            if(!is_dir('modules/webservices/'.$module))
                throw new Exception('Module '.$module.' introuvable !');

            // Test existance fichier config du view module
            if(file_exists('modules/webservices/'.$module.'/config/'.$function.'.ini'))
                $config = parse_ini_file('modules/webservices/'.$module.'/config/'.$function.'.ini', true);
            else
                throw new Exception('Fichier de config '.$function.'.ini du module '.$module.' non trouvée !', 1);

            $mod_config = parse_ini_file('modules/webservices/WebservicesConfig.ini', true);
            ...
            if(file_exists('modules/webservices/'.$module.'/controller/'.$moduleController.'.php'))
                require_once('modules/webservices/'.$module.'/controller/'.$moduleController.'.php');
            else
